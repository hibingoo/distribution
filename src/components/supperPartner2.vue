      ...
      <div class="table">
        ...
        <div class="tableData">
          <div class="tablehead">
            ...
            <div class="effective">
              <span class="effectiveText">老板
                <br>总数</span>
              <!-- <span class="iconfont effectiveicon" :class="{rotate:transform.bossTotal}">&#xe633;</span> -->
            </div>
            <div class="effective">
              <span class="effectiveText">有效
                <br>老板率</span>
              <!-- <span class="iconfont effectiveicon" :class="{rotate:transform.bossPercent}">&#xe633;</span> -->
            </div>
            <div class="effective">
              <span class="effectiveText">有效
                <br>徒弟</span>
            </div>
            <div class="effective" @click.stop="InitializationData('disciplePercent',desc,null,null,null)">
              <span class="effectiveText">有效
                <br>徒弟率</span>
              <span class="iconfont effectiveicon" :class="{rotate:transform.disciplePercent}">&#xe632;</span>
            </div>
          </div>
          <div v-show="!noData">
            <div class="tablebody" v-show="!circle">
              <div class="tableList" v-for="tabledata in list">
                ...
                <div class="disciple">
                  {{tabledata.second_count}}
                </div>
                <div class="effective">
                  {{tabledata.second_percent}}
                </div>
                <div class="effective">
                  {{tabledata.task_count}}
                </div>
                <div class="effective">
                  {{tabledata.percent}}
                </div>
...
export default {
  ...
  methods: {
    ...
    PageLoading() {
      this.loadMoreData(this.desc, this.page++);
    },
    //初始化页面数据
    loadMoreData(order, page) {
      this.noData = false;
      this.circle = true;
      let token = sessionStorage.getItem("token");
      let url = API_ROOT + "/pupil/superPupil";
    ...
    change(type) {
      ...
      if (type == "bossTotal") {
        this.transform.bossTotal = !this.transform.bossTotal;
        this.desc = !this.transform.bossTotal ? "asc" : "desc";
      }
      if (type == "bossPercent") {
        this.transform.bossPercent = !this.transform.bossPercent;
        this.desc = !this.transform.bossPercent ? "asc" : "desc";
      }
      if (type == "disciplePercent") {
        this.transform.disciplePercent = !this.transform.disciplePercent;
        this.desc = !this.transform.disciplePercent ? "asc" : "desc";
      }
    },
    openPicker(type) {
      this.type = type;
